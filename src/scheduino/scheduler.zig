...
pub fn switchProcess(sp: usize) usize {
    Libz.GpIO.DIGITAL_MODE(4, .OUTPUT) catch {};
    Libz.GpIO.DIGITAL_WRITE(4, if (hasJumped and l) .LOW else .HIGH) catch {};

    var new_pid: u8 = 0;
    if (hasJumped) {
        MemState.processes[currentId].stack_pointer = sp;
        new_pid = if (currentId + 1 == MemState.processes.len) 0 else (currentId + 1);
    }
    currentId = new_pid;
    l = if (l) false else true;
    return MemState.processes[new_pid].stack_pointer;
}
